...
case "$COMMAND" in
    *"git commit "*)
        [ "$EXIT_CODE" != "0" ] && exit 0
        echo "REMINDER: You just committed code. If this commit contains a design decision, architectural change, or notable learning, persist it with corvia_write (scope_id: dorea, agent_id: claude-code). Skip if the commit is trivial (typo fix, formatting, etc.)."
        ;;
esac
